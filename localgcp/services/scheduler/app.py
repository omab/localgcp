...
from localgcp.services.scheduler.models import JobListResponse, JobModel
...
app = FastAPI(title="LocalGCP — Cloud Scheduler", version="v1")
...
async def update_job(project: str, location: str, job_id: str, request: Request):
    ...
    if "schedule" in body and body["schedule"]:
        existing["scheduleTime"] = _next_run_time(body["schedule"], datetime.now(timezone.utc))
