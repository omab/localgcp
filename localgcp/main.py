...
logger = logging.getLogger("localgcp")
...
def _build_configs() -> list[tuple[str, uvicorn.Config]]:
    ...
    log_level = os.environ.get("LOCALGCP_LOG_LEVEL", "info").lower()
    ...
    for name, _label, port_fn in _SERVICES:
        log_config = {
            ...
            "formatters": {
                "default": {
                    ...
                    "fmt": f"%(asctime)s  localgcp.{name:<14s}  %(levelprefix)s %(message)s",
                    "use_colors": None,
                },
                "access": {
                    "()": "uvicorn.logging.AccessFormatter",
                    "fmt": f'%(asctime)s  localgcp.{name:<14s}  %(levelprefix)s %(client_addr)s - "%(request_line)s" %(status_code)s',
                },
            },
            "handlers": {
        ...
        configs.append((name, uvicorn.Config(
            app=apps[name],
            host=settings.host,
            port=port_fn(),
            log_level=log_level,
...
async def _serve_all(configs: list[tuple[str, uvicorn.Config]]) -> None:
    ...
    def _shutdown(*_):
        logger.info("Shutting down LocalGCP …")
        for s in servers:
            s.should_exit = True
...
def main() -> None:
    log_level = os.environ.get("LOCALGCP_LOG_LEVEL", "INFO").upper()
    logging.basicConfig(
        level=getattr(logging, log_level, logging.INFO),
        format="%(asctime)s  %(name)-22s  %(levelname)-8s  %(message)s",
    )

    logger.info("Starting LocalGCP  (log level: %s)", log_level)
    logger.info("")
    logger.info("  %-20s → grpc://%s:%d  (PUBSUB_EMULATOR_HOST=%s:%d)",
                "Cloud Pub/Sub gRPC", settings.host, settings.pubsub_port,
                settings.host, settings.pubsub_port)
    for name, label, port_fn in _SERVICES:
        logger.info("  %-20s → http://%s:%d", label, settings.host, port_fn())
    logger.info("")
    logger.info("SDK usage:")
    logger.info("  Pub/Sub (default gRPC):  set PUBSUB_EMULATOR_HOST=localhost:%d", settings.pubsub_port)
    logger.info("  Pub/Sub (REST):          transport='rest', api_endpoint='http://localhost:%d'", settings.pubsub_rest_port)
