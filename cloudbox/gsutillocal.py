"""gsutil-compatible CLI for Cloudbox Cloud Storage emulator.

Mirrors the real gsutil command syntax so existing scripts can target
Cloudbox with minimal changes.  Point at a running Cloudbox instance via
environment variables:

    CLOUDBOX_HOST      (default: localhost)
    CLOUDBOX_GCS_PORT  (default: 4443)
    CLOUDBOX_PROJECT   (default: local-project)
