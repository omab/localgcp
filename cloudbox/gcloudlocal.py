...
Environment variables:
    CLOUDBOX_PROJECT         default project  (default: local-project)
    CLOUDBOX_LOCATION        default location (default: us-central1)
    CLOUDBOX_HOST            emulator host    (default: localhost)
    CLOUDBOX_GCS_PORT        GCS port         (default: 4443)
    CLOUDBOX_PUBSUB_REST_PORT  Pub/Sub REST   (default: 8086)
    CLOUDBOX_FIRESTORE_PORT  Firestore port   (default: 8080)
    CLOUDBOX_SECRETMANAGER_PORT  Secret Mgr   (default: 8090)
    CLOUDBOX_TASKS_PORT      Cloud Tasks port (default: 8123)
"""
...
_BASES: dict[str, str] = {
    "gcs": f"http://{_HOST}:{os.environ.get('CLOUDBOX_GCS_PORT', '4443')}",
    "pubsub": f"http://{_HOST}:{os.environ.get('CLOUDBOX_PUBSUB_REST_PORT', '8086')}",
    "firestore": f"http://{_HOST}:{os.environ.get('CLOUDBOX_FIRESTORE_PORT', '8080')}",
    "secrets": f"http://{_HOST}:{os.environ.get('CLOUDBOX_SECRETMANAGER_PORT', '8090')}",
    "tasks": f"http://{_HOST}:{os.environ.get('CLOUDBOX_TASKS_PORT', '8123')}",
}

DEFAULT_PROJECT = os.environ.get("CLOUDBOX_PROJECT", "local-project")
DEFAULT_LOCATION = os.environ.get("CLOUDBOX_LOCATION", "us-central1")
...
def _build_parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        prog="gcloudlocal",
        description="gcloud-compatible CLI for the Cloudbox emulator.",
